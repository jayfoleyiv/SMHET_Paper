...
palette of materials that can be utilized for hot-carrier mediated photochemistry,
...
observe any overlap between an absorption resonance of the 2nm Pt NS and the dipole-allowed 
...
Pt NC is much smaller ($|{\bf E}|/|{\bf E_0}|$ at $\lambda=200nm$ corresponding to the extinction maximum, see Figure 2 (a), (b), and (c)).
Due to its relatively weak optical interaction, scattering of the Pt NC does not lead to a large density of hot-carriers; small populations
...
incident pulse, which has a peak field strength of $E_0 \approx 614,000,000 \: V/m$,
(see Supplemental Information for more details).
In this case, SMA on the $d=285nm$ nanosphere
...
to the resonances of the $d=285nm$ dielectric NS or the Pt NC.  For example, WGMs supported by the $d=685nm$ NS are three times
...
The hot-carriers driven by WGM also show much more persistent dynamics, and continue to evolve beyond 100 fs (see Figure 2 (e) and (f)).
...
observe in the figures for both metals, that the nanocube cube structures cease to change their orbital 
occupancy after the 20$^{{\rm th}}$ timestep. Similarly we find that the Fabry-Perot structures??
...
each $\psi_{nx,ny,nz}$ can be uniquely labeled $\psi_q$.
Using the above expressions and this unique labeling, the diagonal matrix elements can be evaluated as
\begin{align}
...
\begin{align}
\langle \Phi_0 | \hat{H}(t) | \Phi_i^a \rangle = \langle \psi_i | {\bf E}(t) \cdot {\bf \hat{\mu}} | \psi_a \rangle \\
\langle \Phi_i^a | \hat{H}(t) | \Phi_j^b \rangle = \delta_{ij} \psi_a | {\bf E}(t) \cdot {\bf \hat{\mu}} | \psi_b \rangle - \delta_{ab} \psi_i | {\bf E}(t) \cdot {\bf \hat{\mu}} | \psi_j \rangle.
\end{align} 
