...
the relative importance of particle geometry, material bandstructure, and electronic correlation effects on hot-carrier
...
on hot carrier generation and dynamics in Ag nanospheres compared to geometric effects~\cite{MLK_ACSNano_2014}.} 
...
on noble metal nanoparticles.  Importantly, this method goes beyond a perturbative inclusion of the electric field and enables 
...
This is a unique feature of SMA that is likely unimportant to in plamonic generation of hot-carriers due to the impulsive 
...
a more realistic description of the underlying electronic structure will be an important tool to use, particularly in conjuction with
...
this work neglects excited-state thermalization contributions from electron-electron scattering. %1. Band structure / d electron effects:  The modified text for paragraph 1 of page 5 (page 6 in revised MS) continues to give the impression that free electron and ab initio approaches give the same result.  I think the authors may have misinterpreted Ref 24 (Nordlander et al.): that reference compares free electron models with jellium TD-DFT and finds no difference, but both these methods lack detailed band structure.  What that concludes is that electron exchange/correlations effects at the jellium level are negligible.  (Jellium DFT/TD-DFT calculations are not ab initio.)
%More importantly, in expanding on this point during revision, the authors have mixed up band structure effects including d electrons, which are a single-particle property, with correlation effects, which are many-body.  The correct explanation here is that Ref 24 and 27 show that correlation effects are not particularly important, but Ref 18 and 21 show that band structure effects are very important in gold, but less so in silver due the d band positions.  The rest of the authors response by including silver calculations is greatly appreciated, but the above terminology (correlation vs band structure effects) must be rectified throughout for the updated discussions to be correct.
\textcolor{red}{The underlying simplicity of the Hamiltonian and single-particle basis set also precludes desciption
...
\textcolor{red}{band-structure and many-body effects (elaborate)}, and investigation of these effects on the hot-carrier distributions and dynamics resulting from SMA.  It is expected
